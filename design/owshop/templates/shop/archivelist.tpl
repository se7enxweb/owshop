...
<form action={concat("/owshop/archivelist")|ezurl} method="post" name="Archivelist">
...
<tr>
	...
	<td class="{$Order:sequence}">
    <a href={concat("/owshop/customerorderview/",$Order:item.user_id,"/",$Order:item.account_email)|ezurl}>{$Order:item.account_name|wash}</a>
	</td>
	<td class="{$Order:sequence}">
	...
	<td class="{$Order:sequence}">
	<a href={concat("/owshop/orderview/",$Order:item.id,"/")|ezurl}>[ view ]</a>
	</td>
</tr>
{/section}
</table>
